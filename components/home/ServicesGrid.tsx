...
export default function ServicesGrid() {
  return (
    ...
    >
      <div className="absolute top-1/4 right-0 w-96 h-96 rounded-full bg-[#FF555F]/4 blur-3xl pointer-events-none" />
          ...
          {SERVICES.map((serv, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-[#4AABCA]/4 via-transparent to-[#FF555F]/4 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-[1.25rem]" />
                ...
                <div className="flex flex-col gap-5 relative z-10">
                  ...
                  <div>
                    <h3 className="font-display font-bold text-xl text-slate-100 light:text-[#111827] group-hover:text-[#FF555F] transition-colors uppercase leading-tight">
                      {serv.title}
                    </h3>
                  ...
                  <ul className="flex flex-col gap-2 mt-1">
                    {serv.benefits.map((b, bIdx) => (
                      <li key={bIdx} className="flex items-center gap-2.5 text-sm text-slate-300 light:text-slate-700 font-medium">
                        <CheckCircle2 className="w-4 h-4 text-[#4AABCA] shrink-0" />
                        <span>{b}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="mt-8 pt-5 border-t border-white/6 light:border-t-slate-200 flex justify-between items-center relative z-10">
                  <Link
                    href={serv.href}
                    className="flex items-center gap-1.5 text-sm font-bold text-[#FF555F] hover:gap-2.5 transition-all"
                  >
